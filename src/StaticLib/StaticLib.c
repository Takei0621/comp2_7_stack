...
void initialize(STACK* s, size_t mem_size)
{
	...
	size_t capacity = mem_size / sizeof(int);
	if (capacity == 0) return;

	s->stack_memory = (int*)malloc(capacity * sizeof(int));
	if (!s->stack_memory) return;      //NULLなら終了

	s->stack_pointer = s->stack_memory;
	s->end = s->stack_memory + (ptrdiff_t)capacity;

}


// 確保したメモリを解放する
void finalize(STACK* s)// ToDo: Initializeで確保したメモリを解放しよう
{
...
bool push(STACK* s, int val)// ToDo: valの値をスタックに保存しよう
{
...
bool push_array(STACK* s, int* addr, int num)// ToDo: addrからはじまるnum個の整数をスタックに保存しよう
{
...
// スタックから一つの要素を取り出す
int pop(STACK* s)// ToDo: スタックの最上位の値を取り出して返そう
{

	if (s == NULL) return 0;// 不具合時は0を返す
...
int pop_array(STACK* s, int* addr, int num)// ToDo: スタックからnum個の値を取り出してaddrから始まるメモリに保存しよう
{
